...
| Tool result | `toolCalled` carries `error` and `outputs` (`[{type: text, text}]`). ECA validates a tool call's arguments against the MCP `inputSchema` itself: a call missing a required parameter is answered `error: true` with the text `missing required params: ...` and **never reaches the MCP server** (so Fleet's RPC layer cannot log it) | `tool-finished` and the transcript row carry `error`; a failed call also keeps its outputs text clipped to 600 chars (`output`), since that is the only record of the reason. Successful outputs are not copied |
