...
| Operator answers a commander message without a status | an ordinary `idle` + `finished` after assistant text or tool calls; nothing on the wire tells it apart | a store fact, not a wire one: when no `task-done`/`task-failed`/`task-blocked`/`decision-requested`/`task-paused` from that task and runtime is at or after the message's `created_at`, the supervisor records one actionable `turn-unreported` (`:message-id`) at turn end (design note 23). Barren turns keep `turn-empty`/`turn-failed` |
