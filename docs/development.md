...
| Unresolved owner-facing results (shared by `/fsum` and `/frev`) | [`skills/fsum/scripts/fleet-result.el`](../skills/fsum/README.md): the single Emacs writer of a plain-file checkpoint under `$XDG_DATA_HOME/fleet-result-review/`, plus the commander-transcript collector. **Skill review state, not a Fleet projection**: no schema, RPC, tool or `fleet-store.el` involvement, and Python/browser code must never write it | `skills/fsum/test/fsum-tests.el` (checkpoint, collector, `/fsum` rendering), `skills/frev/test/frev-tests.el` (deep pass and adjudication) |
...
| `prompts/*.md`, fleet `about.md`, `commander/context.md` | Boot messages read these at runtime creation (and only these: `commander/context/` and `commander/archive/` are read on demand, never injected); edits do not retroactively change an agent's context. Commander replacement refreshes commander context/doctrine, **not loaded Lisp**, and leaves operators (and lieutenants) running. |
