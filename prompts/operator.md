...
- Answer a commander's request by publishing an **actionable** status: republish `done` (or `blocked`/
  `needs-decision`) with the answer in `detail`. A `working` status wakes nobody, and chat text reaches
  nobody.
